...
import { connectorPathTileToGlobal, getAnchorTile, getConnectorPath, getConnectorPathPreview } from './renderer';
...
/** First X on the horizontal run from→toX at `y` that already has an edge. */
const findFirstHorizontalConflictX = (
  from: Coords,
  toX: number,
  y: number,
  otherEdges: Set<string>
): number | null => {
  if (from.x === toX) return null;
  const sign = Math.sign(toX - from.x) || 1;
  const maxSteps = Math.abs(toX - from.x) + 2;
  for (let i = 0, x = from.x; i < maxSteps; i += 1, x += sign) {
    const next = x + sign;
    if (otherEdges.has(edgeKey({ x, y }, { x: next, y }))) {
      return x;
    }
    if (next === toX) break;
  }
...
/**
 * Sideways-first L, then a minimal 45° lane change just before the first
 * overlapping cable — only shifts enough to clear that run (no shared
 * drop-under corridor).
 */
const horizontalFirst45DodgePatterns = (
  from: Coords,
  to: Coords,
  otherPaths: Coords[][],
  laneIndex: number
): HelperPattern[] => {
  const patterns: HelperPattern[] = [];
  if (from.x === to.x || from.y === to.y) {
    return patterns;
  }

  // 1) Plain sideways-first L (right/left at source Y, then vertical).
  patterns.push([{ x: to.x, y: from.y }]);

  const otherEdges = buildOtherEdgesSet(otherPaths);
  const signX = Math.sign(to.x - from.x) || 1;
  // Prefer dodging toward the destination's Y (usually down to the switch).
  const destYSign = to.y === from.y ? 1 : to.y > from.y ? 1 : -1;
  const ySigns =
    laneIndex % 2 === 0
      ? [destYSign, -destYSign]
      : [-destYSign, destYSign];

  const conflictX = findFirstHorizontalConflictX(
    from,
    to.x,
    from.y,
    otherEdges
  ...
  // Only emit 45° dodges when there is a real conflict on this sideways run.
  if (conflictX == null) {
    return patterns;
  }

  ySigns.forEach((ySign) => {
    // Minimal shift first (1 tile) — only escalate if needed by the caller loop.
    for (let mag = 1; mag <= 4; mag += 1) {
      const laneY = from.y + ySign * mag;
      // Start 45° so we finish `mag` diagonal steps exactly at the conflict X.
      let diagStartX = conflictX - signX * mag;
      if (signX > 0 && diagStartX < from.x) continue;
      if (signX < 0 && diagStartX > from.x) continue;
      // Diagonal must stay within the sideways span (or start at `from`).
      if (signX > 0 && conflictX > to.x) continue;
      if (signX < 0 && conflictX < to.x) continue;

      const diagEnd = { x: conflictX, y: laneY };
      const approach = { x: to.x, y: laneY };

      const helpers: Coords[] = [];
      const pushUnique = (tile: Coords) => {
        if (CoordsUtils.isEqual(tile, from) || CoordsUtils.isEqual(tile, to)) {
          return;
        }
        const prev = helpers[helpers.length - 1];
        if (prev && CoordsUtils.isEqual(prev, tile)) return;
        helpers.push(tile);
      };

      // When conflict is right next to the source, start the 45° from `from`.
      if (diagStartX !== from.x) {
        pushUnique({ x: diagStartX, y: from.y });
      }
      pushUnique(diagEnd);
      pushUnique(approach);
      if (helpers.length > 0) {
...
/**
 * 2D auto / simple paths: keep the current route when clear; when it shares
 * edges with already-routed cables, prefer sideways-first exit + a minimal
 * 45° dodge just before the conflict (no shared drop-under corridor).
 */
export const separatePreviewPathFromOthers = ({
  anchors,
  view,
  modelItems,
  otherPaths,
  ...
  const currentTiles = buildPreviewPathTiles(anchors, view, modelItems);
  if (!currentTiles) {
    return anchors;
  }
  ...
  if (
    ...
    // Do not materialize a side-first mid WP when the path is already clear —
    // preview draws that L without stored waypoints, and new IDs break drag.
    return anchors;
  }

  // Existing mid WPs: nudge overlapping runs before rebuilding from ports.
  if (anchors.length > 2) {
    let current = anchors;
    for (let iter = 0; iter < MAX_RESOLVE_ITERS; iter += 1) {
      const tiles = buildPreviewPathTiles(current, view, modelItems);
      if (!tiles || !hasEdgeOverlapWithOthers(tiles, otherPaths)) {
        return current;
      }
      const runs = findOverlapRuns(tiles, otherPaths);
      ...
      current = applyBumpToAnchors({
        ...
        globalTiles: tiles,
        run: runs[0],
        offset: bumpOffsetForRun(tiles, runs[0], magnitude, sign),
        view,
        modelItems
      });
    }

    const bumpedTiles = buildPreviewPathTiles(current, view, modelItems);
    if (
      bumpedTiles &&
      !hasEdgeOverlapWithOthers(bumpedTiles, otherPaths)
    ) {
      return current;
    }
  }
  ...
  // Sideways-first L, then minimal 45° dodge just before the overlapping cable.
  const dodgePatterns = horizontalFirst45DodgePatterns(
    from,
    to,
    otherPaths,
    laneIndex
  );
  for (const helpers of dodgePatterns) {
    if (helpers.length === 0) continue;
    const candidate = makeCandidate(helpers);
    const tiles = buildPreviewPathTiles(candidate, view, modelItems);
    if (!tiles) continue;
  ...
  // Vertical-trunk (or other) overlap missed by horizontal conflict scan.
  if (bestOverlap > 0) {
    const sideFirstHelpers =
      from.x === to.x || from.y === to.y ? [] : [{ x: to.x, y: from.y }];
    const sideFirstTiles =
      sideFirstHelpers.length > 0
        ? buildPreviewPathTiles(makeCandidate(sideFirstHelpers), view, modelItems)
        : currentTiles;
    const runs = sideFirstTiles
      ? findOverlapRuns(sideFirstTiles, otherPaths)
      : [];
    if (runs.length > 0) {
      const conflictTile = sideFirstTiles![runs[0].start];
      const signX = Math.sign(to.x - from.x) || 1;
      const ySign = to.y >= from.y ? 1 : -1;
      for (let mag = 1; mag <= 4; mag += 1) {
        const laneY = from.y + ySign * mag;
        const diagStartX = conflictTile.x - signX * mag;
        if (signX > 0 && diagStartX < from.x) continue;
        if (signX < 0 && diagStartX > from.x) continue;
        const helpers: Coords[] = [];
        if (diagStartX !== from.x) {
          helpers.push({ x: diagStartX, y: from.y });
        }
        helpers.push({ x: conflictTile.x, y: laneY });
        if (conflictTile.x !== to.x || laneY !== to.y) {
          helpers.push({ x: to.x, y: laneY });
        }
        const candidate = makeCandidate(helpers);
        const tiles = buildPreviewPathTiles(candidate, view, modelItems);
        if (!tiles) continue;
